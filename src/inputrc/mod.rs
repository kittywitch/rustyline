...
use self::chars::{ctrl, meta, parse_char_name};
...
/// Load inputrc from the config locations
pub fn load_inputrc() -> std::io::Result<Vec<Directive>> {
    let inputrc_etc = Path::new("/etc/inputrc");
    let inputrc_env = env::var_os("INPUTRC").map(PathBuf::from);
    let inputrc_env = inputrc_env.as_ref().map(|p| p.as_path());
    let inputrc_home = env::var_os("HOME").map(|home| PathBuf::from(home).join("/.inputrc"));
    let inputrc_home = inputrc_home.as_ref().map(|p| p.as_path());

    let inputrc_path = [inputrc_env, inputrc_home, Some(inputrc_etc)]
        .iter()
        .copied()
        .filter_map(|p| p)
        .filter(|p| p.exists())
    ...
    inputrc_path
        .map(|path| fs::read_to_string(path).map(|str| parse_text(path, &str)))
...
#[cfg(test)]
mod test {
    use super::{parse_text, Directive};
    use crate::inputrc::command::Command;
